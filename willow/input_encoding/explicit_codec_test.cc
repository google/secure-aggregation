...
TEST(CodecFactoryTest, ValidateInputSpecFlattenedDomainSize) {
  InputSpec input_spec;
  ...
  EXPECT_THAT(CodecFactory::ValidateExplicitCodecInputSpec(input_spec, 17),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("Global output domain size exceeds")));
  SECAGG_EXPECT_OK(
      CodecFactory::ValidateExplicitCodecInputSpec(input_spec, 18));
}
...
TEST(CodecFactoryTest, ValidateInputSpecWithTimeDomain) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
  ...
  // Valid spec
  SECAGG_EXPECT_OK(CodecFactory::ValidateExplicitCodecInputSpec(input_spec));
...
TEST(CodecFactoryTest, EncodeTimeDomainFailsWithoutEncodingTime) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
  ...
  metric_data["metric1"] = {10};
  GroupData group_by_data;
  group_by_data["time"] = {"2026-01-08T12:00:00Z"};

  // Encode without encoding_time should fail
  EXPECT_THAT(encoder->Encode(group_by_data, metric_data),
              StatusIs(absl::StatusCode::kFailedPrecondition,
                       HasSubstr("encoding_time is required")));
}

TEST(CodecFactoryTest, EncodeTimeDomain) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
  ...
  SECAGG_ASSERT_OK_AND_ASSIGN(
      ...
      CodecFactory::CreateExplicitCodec(input_spec, encoding_time));
...
TEST(CodecFactoryTest, DecodeTimeDomain) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
  ...
  SECAGG_ASSERT_OK_AND_ASSIGN(
      ...
      CodecFactory::CreateExplicitCodec(
          input_spec, /*encoding_time=*/std::nullopt, decoding_anchor_time));
  ...
  // Bucket 6 is an invalid timestamp.
  std::string expected_time_str = "1970-01-09";
  EXPECT_THAT(decoded_data.group_data.at("time"),
              ElementsAre(expected_time_str, kInvalidTimestamp));
}

TEST(CodecFactoryTest, EncodeThenDecodeLocalTime) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
  ...
  SECAGG_ASSERT_OK_AND_ASSIGN(
      ...
      CodecFactory::CreateExplicitCodec(input_spec, encoding_time1));
  ...
  SECAGG_ASSERT_OK_AND_ASSIGN(
      ...
      CodecFactory::CreateExplicitCodec(input_spec, encoding_time2));
...
TEST(CodecFactoryTest, EncodeThenDecodeAbsoluteTime) {
  InputSpec input_spec;
  auto* metric_spec = input_spec.add_metric_vector_specs();
  metric_spec->set_vector_name("metric1");
  metric_spec->set_data_type(InputSpec::INT64);
