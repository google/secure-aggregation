...
FfiStatus SerializeRnsPolynomialToBytes(const RnsPolynomialWrapper* poly,
  ...
  std::string buffer;
  serialized->SerializeToString(&buffer);
...
FfiStatus DeserializeRnsPolynomialFromBytes(
  ...
  if (!serialized_poly_proto.ParseFromString(absl::string_view(
          reinterpret_cast<const char*>(serialized_poly.data()),
          serialized_poly.size()))) {
    return MakeFfiStatus(absl::InvalidArgumentError(
